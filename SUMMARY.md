...
flowchart TD
    ...
    subgraph Peripherals ["Presentation & Utilities"]
        ...
        Factories["ItemFactory, CreatureFactory & CreatureSpawner"]
    end
...
| **MatingSystem** | `226` | [MatingSystem.gd](file:///d:/GODOT/spire/modules/creature/systems/MatingSystem.gd) | **Reproduction, Fecundity, Courtship & Pair-Bond Fidelity.** Simulates biological sex (`Gender.MALE`, `Gender.FEMALE`), male/female genital limbs, libido drive accumulation, partner evaluation via trait matching, gestation timers, and birth spawning scaled by female spawnrate. Features a continuous **Pair-Bond Fidelity / Monogamy Level tendency ($0.0 \dots 1.0$)**: applies loyalty bonuses towards bonded partners, extra-pair mating penalties scaled by monogamy tendency, strict fidelity rejection for high-monogamy creatures ($0.70+$), mate-guarding proximity enforcement ($0.35+$), living bond preservation, and widowhood remating upon partner death. |
| **SocialSystem** | `227` | [SocialSystem.gd](file:///d:/GODOT/spire/modules/creature/systems/SocialSystem.gd) | **Social Dynamics, Pack Formation & Herd Cohesion.** Runs on a 5-tick cadence. Prunes dead bond links, computes peer counts within species social radius, identifies closest companions, and calculates local herd centroids. Dynamically increases sociability drive (loneliness) when isolated and satisfies it when clustered, outputting social sensory signals for AI perception. |
| **CreatureAISystem** | `230` | [CreatureAISystem.gd](file:///d:/GODOT/spire/modules/creature/systems/CreatureAISystem.gd) | **Vector Utility AI & Cognitive Planner.** Samples spatial signals and tile affordances at the creature's position. Computes dot-product utility scores against state-space vector embeddings (`MindEmbeddings`) for actions (`FLEE`, `EAT`, `DRINK`, `REST`, `MATE`, `SOCIALIZE`, `WANDER`, `IDLE`). Integrates diet-aware compatible food targeting, companion approach planning, juvenile mother-following, and Boids-style herd centroid cohesion during wandering. |
...
- [CreatureSpawner.gd](file:///d:/GODOT/spire/modules/creature/systems/CreatureSpawner.gd): **Extensible Spawning & Starter Population.** Spawns creatures individually or in batches from configuration dictionaries. Enforces spatial criteria (vegetation affordances, safety, collision avoidance, cluster proximity) and defaults to introducing starter Male & Female Grazers into the living simulation.
...
- [InspectorHUD.gd](file:///d:/GODOT/spire/ui/InspectorHUD.gd): **Tile & Entity Inspector.** Interactive split/tabbed UI (`I` or Left-Click). Provides deep telemetry for environment tiles (terrain, climate, active matter, vegetation, items, sensory signals, affordance flags) and creature internals (thoughts, drives, utility scores, anatomical health, blood volume, active traits). Includes camera snapping and forward/backward cycling navigation buttons to quickly locate and inspect other creatures available on the map.
