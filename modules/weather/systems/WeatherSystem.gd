...
const MAX_STRENGTH: Array = [0.10, 0.40, 0.72, 1.00]

## Human-readable names for console output.
const CONDITION_NAMES: Array = ["CALM", "BREEZY", "WINDY", "STORM"]

## Transition weights (must sum to 100): CALM, BREEZY, WINDY, STORM.
## Keeps the world mostly breezy/windy; storms are rare.
const TRANSITION_WEIGHTS: Array = [12, 42, 36, 10]
...
func initialize() -> void:
	...
	_condition_duration = 200 + _rng.randi_range(0, 300)
...
func tick(_tick_number: int) -> void:
	...
	if _condition_timer >= _condition_duration:
		_condition_timer = 0
		_condition_duration = 150 + _rng.randi_range(0, 400)
...
func _transition_condition() -> void:
	...
	for i: int in range(TRANSITION_WEIGHTS.size()):
		...
		if roll < acc:
			...
			break
	world.wind_condition = _condition
	print("[Weather] Condition → %s  (max strength %.0f%%)" % [
		CONDITION_NAMES[_condition],
		(MAX_STRENGTH[_condition] as float) * 100.0
	])
